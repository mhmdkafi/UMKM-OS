...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/dashboard/summary?business_id=${encodeURIComponent(user.business_id)}`)
      .then(res => res.json())
  ...
  const dataSales = recentTx.slice(0, 7).reverse().map((tx: any, i: number) => ({
    name: new Date(tx.created_at).toLocaleDateString('id-ID', { weekday: 'short' }),
    revenue: tx.total_amount,
    profit: Math.round(tx.total_amount * 0.4),
  }));
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <KpiCard 
          ...
          change={`${kpi.totalTransactions} transaksi`} 
          isPositive={true} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-semibold text-slate-800">Transaksi Terakhir (Chart)</h3>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dataSales} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" name="Estimasi Laba" dataKey="profit" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorProfit)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
